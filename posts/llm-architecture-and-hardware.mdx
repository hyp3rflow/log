---
title: "LLM 아키텍처와 하드웨어의 공진화 — GQA에서 HBM4까지"
date: 2026/02/21
...
# LLM 아키텍처와 하드웨어의 공진화 — GQA에서 HBM4까지

## 1. Transformer와 메모리 병목

Transformer의 구조와 각 구성요소의 역할은 [이전 글](/posts/transformer-anatomy)에서 다뤘다. 여기서는 이 구조가 **왜 메모리 병목을 만드는지**부터 시작한다.

Self-Attention의 시간 복잡도는 O(n² · d)이다. QKᵀ 행렬의 크기는 n × n이므로, 시퀀스 길이가 길어지면 **메모리 사용량이 제곱으로 증가**한다.

### 1.1 KV Cache — 추론 시 메모리 소비의 주범
...
위치 인코딩의 발전(Sinusoidal → RoPE → ALiBi)은 [이전 글](/posts/transformer-anatomy#8-positional-encoding)에서 상세히 다룬다. 여기서는 메모리와의 연결만 짚는다.

RoPE의 NTK-aware scaling이나 YaRN 같은 기법으로 학습 시 4K 토큰이던 컨텍스트를 32K, 128K로 확장할 수 있게 되었다. 그러나 시퀀스 길이가 4K에서 128K로 32배 늘어나면, KV Cache도 32배 증가한다. 위치 인코딩의 발전 — 특히 RoPE의 스케일링 기법 — 이 없었다면 이 정도의 컨텍스트 확장은 불가능했을 것이고, 동시에 이 확장이 만들어낸 메모리 수요가 하드웨어 혁신을 촉진했다.

### 2.3 Mixture of Experts (MoE) — 파라미터는 늘리고 연산량은 유지

Shazeer et al.(2017)이 제안한 MoE 개념[^6]은 최근 Mixtral 8x7B[^7], GPT-4 (추정), DeepSeek-V2 등에서 활발히 사용된다.
...
Dao et al.(2022)이 제안한 Flash Attention[^8]은 attention 연산의 **메모리 IO**를 최적화한다.
...
Flash Attention 2[^9] (2023)는 parallelism을 개선하여 A100에서 이론 최대 대역폭의 72%를 달성했다. Flash Attention 3 (2024)는 H100의 FP8 Tensor Core와 비동기 연산을 활용한다.
...
JEDEC 표준 기준으로 HBM의 세대별 스펙은 다음과 같다[^10]:
...
삼성이 발표한 **HBF (High Bandwidth Flash)**는 **NAND 플래시 메모리를 HBM과 유사한 방식으로 3D 적층**하여 대역폭을 극대화한 새로운 유형의 메모리이다[^11].
...
데이터센터 GPU의 메모리 스펙 변화는 LLM 발전과 직접 연결된다[^12]:
...
[^6]: Shazeer, N., et al. "Outrageously Large Neural Networks: The Sparsely-Gated Mixture-of-Experts Layer." ICLR 2017. [arXiv:1701.06538](https://arxiv.org/abs/1701.06538)

[^7]: Jiang, A.Q., et al. "Mixtral of Experts." Mistral AI, 2024. [arXiv:2401.04088](https://arxiv.org/abs/2401.04088)

[^8]: Dao, T., et al. "FlashAttention: Fast and Memory-Efficient Exact Attention with IO-Awareness." NeurIPS 2022. [arXiv:2205.14135](https://arxiv.org/abs/2205.14135)

[^9]: Dao, T. "FlashAttention-2: Faster Attention with Better Parallelism and Work Partitioning." 2023. [arXiv:2307.08691](https://arxiv.org/abs/2307.08691)

[^10]: JEDEC. High Bandwidth Memory (HBM) DRAM standards: JESD235, JESD235A, JESD235B, JESD235C, JESD235D. [jedec.org](https://www.jedec.org/)

[^11]: Samsung Electronics. "Samsung Develops Industry's First High Bandwidth Flash (HBF) Memory for On-Device AI." Samsung Semiconductor Newsroom.

[^12]: NVIDIA. Data Center GPU product specifications. [nvidia.com/en-us/data-center](https://www.nvidia.com/en-us/data-center/)
