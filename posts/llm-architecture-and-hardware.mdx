...
원래 Transformer는 **sinusoidal positional encoding**을 사용했다. 학습 시 본 시퀀스 길이를 넘어서면 일반화가 어려웠다.

**Rotary Position Embedding (RoPE)**는 Su et al.(2021)이 제안했다[^6]. Query와 Key에 회전 행렬을 적용하여 상대적 위치 정보를 인코딩한다. 핵심 수식은:

**f(x_m, m) = x_m · e^(imθ)**

RoPE의 장점은 **NTK-aware scaling**, **YaRN** 등의 기법으로 학습 시 컨텍스트 길이를 넘어서는 외삽(extrapolation)이 가능하다는 것이다. LLaMA, Qwen, Mistral 등 대부분의 최신 모델이 RoPE를 사용한다.

**ALiBi (Attention with Linear Biases)**는 Press et al.(2022)이 제안했다[^7]. 위치 임베딩을 아예 제거하고, attention score에 거리에 비례하는 선형 bias를 더한다:

**softmax(qᵢ · kⱼ − m · |i − j|)**

ALiBi는 학습 없이도 더 긴 시퀀스로 외삽이 가능하다. BLOOM (176B) 모델이 ALiBi를 사용했다[^8].

긴 컨텍스트 지원은 메모리 관점에서 중요하다. 시퀀스 길이가 4K에서 128K로 32배 늘어나면, KV Cache도 32배 증가한다. 위치 인코딩의 발전이 없었다면 이 정도의 컨텍스트 확장은 불가능했을 것이다.
...
### 3.3 HBF (High Bandwidth Fan-out)

SK하이닉스가 2024년에 발표한 **HBF (High Bandwidth Fan-out)**는 기존 TSV 기반 HBM의 한계를 보완하는 패키징 기술이다[^14]. 기존에는 실리콘 인터포저(silicon interposer)를 통해 HBM 스택과 프로세서를 연결했다. 인터포저는 고가이고 면적 제약이 있다.

HBF는 **팬아웃 웨이퍼 레벨 패키징(FOWLP)** 기술을 사용하여 인터포저 없이 HBM 다이를 직접 재배선층(RDL)으로 연결한다. 이로 인해:

- 패키징 비용 절감
- 더 많은 HBM 스택을 프로세서에 연결 가능
- 열 관리 개선 (적층 높이 감소 가능)

HBF는 HBM4 세대부터 본격 적용될 것으로 예상된다.
...
[^14]: SK hynix. "HBM Roadmap and Advanced Packaging Technologies." Hot Chips 2024.
