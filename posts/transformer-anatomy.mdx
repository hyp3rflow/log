...
입력 시퀀스의 각 토큰은 먼저 임베딩 벡터 <Math expr='x in RR^d' />로 변환된다. 이 **동일한 입력**에 서로 다른 가중치 행렬을 곱하여 세 가지 벡터를 만든다:
...
<Math expr='W_Q, W_K, W_V in RR^(d times d_k)' />는 **학습되는 파라미터(learnable parameters)**이다. 이 행렬들은 사람이 설계하는 것이 아니라, 모델 학습 과정에서 자동으로 결정된다:
...
1. <Math expr='Q K^T' /> (n × n 행렬): 모든 토큰 쌍 사이의 **유사도 점수**를 계산한다. (i, j) 원소는 "토큰 i의 질문이 토큰 j의 광고와 얼마나 맞는가"를 나타낸다.
2. <Math expr='slash sqrt(d_k)' />: 내적 값이 차원 <Math expr='d_k' />에 비례하여 커지는 것을 방지한다. 스케일링 없이 softmax에 넣으면 큰 값에 확률이 집중되어 그래디언트가 소실된다.
3. **softmax**: 각 행을 확률 분포로 변환한다. 토큰 i가 다른 토큰들에 얼마나 **주의(attention)**를 기울이는지의 가중치가 된다.
...
4. 모든 head의 출력을 **concat** 후, 출력 가중치 <Math expr='W_O in RR^(d times d)' />를 곱한다:
